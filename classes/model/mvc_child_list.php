...
class mvc_child_list {
    ...
    //to save meeting and tasks
    public function save_children_meeting() {
        if (!is_null($this->children)) {
            foreach ($this->children as $child) {
                $child->savenewtask();
            }
    ...
    /**
     * Delete all child records (without grade)
     */
    public function delete_children_meeting() {
        $this->load();
        foreach ($this->children as $child) {
            $child->delete_meeting();
        }
    }
