@mod @mod_mrproject
Feature: Teachers can edit other teacher's tasks only by permission
  In order to edit another teacher's task
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | mrproject | Test mrproject | n     | C1     | mrprojectt | 0         | 3        | 100   |
    And the following "permission overrides" exist:
      | capability                               | permission | role    | contextlevel | reference |
      | mod/mrproject:canseeotherteachersbooking | Allow      | teacher | Course       | C1        |
    And I log in as "edteacher1"
    And I am on "Course 1" course homepage
    And I add a meeting 10 days ahead at 0300 in "Test mrproject" mrproject and I fill the form with:
      | Location     | Office ed1 |
      | studentid[0] | Student 1  |
    And I log out
    And I log in as "neteacher1"
    And I am on "Course 1" course homepage
    And I add a meeting 10 days ahead at 0400 in "Test mrproject" mrproject and I fill the form with:
      | Location     | Office ne1 |
      | studentid[0] | Student 2  |
    And I log out
    And I log in as "neteacher2"
    And I am on "Course 1" course homepage
    And I add a meeting 10 days ahead at 0500 in "Test mrproject" mrproject and I fill the form with:
      | Location     | Office ne2 |
      | studentid[0] | Student 3  |
    And I log out

  @javascript
  Scenario: Editing teachers edit all tasks, nonediting teachers only their own
    ...
    And "Notes for task (visible to student)" "field" should exist
    ...
    And "Notes for task (visible to student)" "field" should not exist
  ...
  Scenario: Attended boxes can be edited if the teacher has permission
    ...
    And I set the following system permissions of "Non-editing teacher" role:
      ...
      | mod/mrproject:editallattended | Allow      |
    And I log out
  ...
  Scenario: Grade boxes can be edited if the teacher has permission
    ...
    And I set the following system permissions of "Non-editing teacher" role:
      | capability                  | permission |
      | mod/mrproject:editallgrades | Allow      |
    And I log out
  ...
  Scenario: Comment boxes can be edited if the teacher has permission
    ...
    And I set the following system permissions of "Non-editing teacher" role:
      | capability                 | permission |
      | mod/mrproject:editallnotes | Allow      |
    And I log out
    ...
    And "seen[]" "checkbox" should not exist in the "5:00 AM" "table_row"
    When I click on "//a[text()='Student 2']" "xpath_element" in the "4:00 AM" "table_row"
    Then the "Notes for task (visible to student)" "field" should be enabled
    And the "Confidential notes (visible to teacher only)" "field" should be enabled
    When I set the following fields to these values:
    | Notes for task (visible to student)   | notes-vis |
    ...
    And the "Attended" "checkbox" should be disabled
    And the "Notes for task (visible to student)" "field" should be enabled
    And the "Confidential notes (visible to teacher only)" "field" should be enabled
    When I set the following fields to these values:
    | Notes for task (visible to student)   | notes-vis-3 |
